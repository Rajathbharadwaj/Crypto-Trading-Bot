...
def run_watchdog():
    """Run the ETH bot with a watchdog that will restart it if it crashes"""
    log_message("Starting ETH Bot Watchdog")
    ...
    while restart_count <= max_restarts:
        ...
        # Log restart attempt
        log_message(f"Starting ETH bot (attempt {restart_count}/{max_restarts+1})")
        
        # Start the bot process
        try:
            # Start the bot as a subprocess without piping output so it shows directly in console
            process = subprocess.Popen([sys.executable, "run_eth_ema_psar.py"])
            
            log_message(f"ETH bot started with PID {process.pid}")
            ...
            log_message(f"ETH bot process exited with code {exit_code}")
            
            # If exit code is 0, it was a clean exit
            if exit_code == 0:
                log_message("ETH bot exited cleanly. Not restarting.")
                break
            
            # Otherwise, it crashed
            log_message("ETH bot crashed. Preparing to restart...")
